        ...
        try
        {
            if (info.Length() >= 1)
            {
                if (info[0]->IsString())
                {
                    ...
                    // In the background, it triggers memory allocation (stack allocating,
                    // but std:string is also dynamically allocating memory in the heap)
                    // We want to avoid heap allocation to ensure more performant code.
                    // See
                    // https://github.com/mapbox/cpp/blob/master/glossary.md#stack-allocation
                    // and
                    ...
                    // This line is where HelloObjectAsync takes ownership of "name" with
                    // the use of move semantics.
                    // Then all later usage of "name" are passed by reference (const&),
                    // but the acutal home or address in memory
                    // will always be owned by this instance of HelloObjectAsync.
                    // Generally important to know what has ownership of an object.
                    // When a object/value is a member of a class (like "name"), we know
                    // the class (HelloObjectAsync) has full control of the scope of the
                    // object/value.
                    // This avoids the scenario of an object/value being destroyed or
