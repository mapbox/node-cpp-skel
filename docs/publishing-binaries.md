...
**Option A:** Require your module in downstream applications like:
...
```js
"your-module": "0.1.0-alpha1",
```

For npm dev releases, it’s good to use the `--tag <something>` to avoid publishing to the latest tag. If you run `npm publish` then `0.1.0-alpha` is what anyone running `npm install vtquery --save` will receive. 
